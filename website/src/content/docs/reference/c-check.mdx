...
Source of truth: `skills/c-check/SKILL.md`. For the *why* behind the diagnostics, read the [Diagnostics deep dive](/cadence/course/diagnostics/).
...
Use `/c-check` before approving a design, before running `/c-plan` or `/c-execute`, or after a long brainstorm — any time you want a sanity check on whether a doc is ready for the next phase. It is an off-pipeline, read-only diagnostic. Use `/c-find-bugs` instead when you want concrete code defects, not a "is this good?" judgment.
...
- Severity is applied conservatively — Critical is reserved for things that block the next phase outright.

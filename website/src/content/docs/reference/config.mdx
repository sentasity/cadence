...
A committed "local" file is just a second team config with a confusing name. `.cadence/config.local.yaml` must be in your repo's `.gitignore`; when Cadence finds the file present but not ignored, it adds the ignore line for you and says so.
</Aside>

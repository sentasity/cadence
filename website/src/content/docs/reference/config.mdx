...
| --- | --- | --- | --- |
| `config_version` | Schema version of this file; used to detect when a migration is needed. | `4` | Integer |
...
Five optional shell-command hooks let a repo plug its own scripts into fixed lifecycle points. Every hook is `null` by default, and a `null` hook means that lifecycle phase simply does not exist; with all hooks empty, `/c-worktree` runs its generic core only.
...
- **Working directory.** `provision`, `port_assign`, and `dev_server` run with cwd = `$WT_PATH`. `port_release` runs with cwd = `$MAIN_ROOT` (the worktree may already be removed). `deploy_guard` runs in the directory the deploy was attempted from: that location is what it judges.
- **Stdout.** Only `port_assign`'s stdout carries meaning (the assigned port, persisted as `git config branch.<branch>.devPort`). Every other hook's output is informational and shown to you.
- **Non-zero exit.** `provision`: surfaced, with an offer to remove the half-created worktree. `port_assign`: surfaced; the lifecycle continues without a port, and the dev-server phase is then unavailable. `dev_server`: surfaced. `port_release`: surfaced, but cleanup continues. `deploy_guard`: the deploy is refused, with an explanation of the merge-first-then-deploy-from-main flow.

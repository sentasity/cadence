...
The whole pipeline, with the gate on each arrow (the thing *you* do to let the work advance) and the diagnostics sitting off to the side:
...
flowchart LR
    ...
    subgraph diag ["Diagnostics — off-pipeline, run any time"]
      direction LR
      C["/c-check"]
      F["/c-find-bugs"]
      X["/c-explain"]

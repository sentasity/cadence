...
There are also three **diagnostic** commands you can run any time, off to the side of the pipeline: `/c-check`, `/c-find-bugs`, and `/c-explain`. That's nine commands in total. You meet the diagnostics in the last deep dive. The plugin also installs one **utility** command that sits outside the pipeline entirely: `/c-worktree`, for driving isolated git worktrees by hand.

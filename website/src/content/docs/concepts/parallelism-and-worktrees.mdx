...
description: How /c-execute schedules work, using a dependency DAG of tasks run as parallel lanes in isolated git worktrees, guarded so two lanes never touch the same files at once.
sidebar:
  order: 3
---
...
flowchart TB
    ...
    review --> merge["merge back per execute.integrate<br/>(default rebase-ff)"]

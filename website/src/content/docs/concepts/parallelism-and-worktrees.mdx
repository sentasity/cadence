...
<Aside type="note" title="Worktrees aren't only for lanes anymore">
The same worktree machinery is also driven interactively by the **[/c-worktree](/cadence/reference/c-worktree/)** utility command: you create a worktree by hand, work in it, and merge it back yourself. A shared merge lock (`worktree.merge_lock`, on by default) serializes any two concurrent merges into the same integration branch, so a `/c-worktree` merge and a `/c-execute` lane landing cannot collide.
</Aside>

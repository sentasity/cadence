...
- **draft → in-review** — triggered by **[/c-design](/cadence/course/design/)** after it finishes its own self-review pass. The design is written and internally checked; now it's ready for *your* eyes.
...
- **approved → completed** — offered by **[/c-validate](/cadence/course/validate/)** once the plan built from this design reaches `completed`. Cadence offers it; you confirm.
...
- **draft → in-progress** — triggered when **[/c-execute](/cadence/course/execute/)** starts running the plan.
- **in-progress → implemented** — triggered by **[/c-execute](/cadence/course/execute/)**, but only after the completion **audit passes**. The code is written *and* checked against its own plan before the plan is called implemented.
- **implemented → completed** — triggered by **[/c-validate](/cadence/course/validate/)** once it walks the plan's validation checklist and everything passes clean (after you've deployed).

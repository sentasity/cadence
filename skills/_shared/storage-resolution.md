...
**The content write path's credential** is `NOTION_TOKEN`: the token of a Notion internal integration the user creates (https://www.notion.so/profile/integrations) and, like the MCP's integration, shares with `root_page` (sharing cascades to descendants). `scripts/notion-write.js` reads it from the environment only. The token value never appears in `.cadence/config.yaml`, `.cadence/config.local.yaml`, frontmatter, chat output, or logs. A missing `NOTION_TOKEN` is a hard stop with an actionable message (the script's exit 2), the same shape as the missing-MCP hard fail: never degrade to an MCP content write, and never fall back to the filesystem.

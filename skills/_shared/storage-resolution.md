...
| `write_doc(type, slug, slot, content)` | type, slug, slot id, markdown body | nothing | write `<slot>.md` in the folder | content authored in Notion-flavored Markdown (translate any obsidian callouts that slipped in — pre-send guard), body written to a local file and pushed by `scripts/notion-write.js`, which guards, writes, and verifies |
...
2. **Confirm the required tools are present.** Verify the located MCP exposes the primitives the storage layer needs (create page, create database, query or search, fetch, update page properties and make small content edits). A missing primitive is treated as "no usable Notion MCP" (see below), not worked around. Doc-body writes are not on this list: they go through `scripts/notion-write.js`, not the MCP (see [[#Content write path]]).
3. **Bind operations to tools.** Map each abstract operation in [[#The abstract operation set]] onto the official MCP's documented tool: `create_artifact` (the database row and its properties) via `notion-create-pages`; `resolve_links` and `tick` via `notion-update-page`; `read_artifact` via `notion-fetch`; `query` and `resolve` via `notion-query-data-sources`; database creation via `notion-create-database`. Content-bearing `write_doc` deliberately binds to no MCP tool: doc bodies go through `scripts/notion-write.js` instead (see [[#Content write path]]), and only small-argument calls remain on the MCP.
...
**Every content-bearing write goes through `scripts/notion-write.js` (invoked as `node "${CLAUDE_PLUGIN_ROOT}/scripts/notion-write.js"`), never through an MCP content call.** This is a hard rule on the notion branch, binding on every doc body regardless of size: `write_doc` writes the full body to a local file, then invokes the script, which pushes the file's content to Notion over the REST markdown endpoints (`Notion-Version: 2026-03-11`) and verifies the result before exiting.

**Why a script.** The streamed-tool-argument path is the one that corrupts. Claude Code's streaming argument accumulator can silently shear a large string argument ([anthropics/claude-code#67765](https://github.com/anthropics/claude-code/issues/67765), open and unfixed): the in-flight string token is dropped when the buffer ends mid-string, the JSON is repaired into something syntactically valid, and a short or mangled body reaches the MCP while the call *succeeds*. Cadence v0.13.2 managed that exposure with an ~8k-character cap, chunked create-then-append, and a prose-driven read-back ritual. The script removes the exposure instead of managing it: the body travels from a local file to Notion in a plain HTTP request and never rides in a streamed tool argument, so there is no cap, no chunking, and no read-back ritual for skills to enforce. Notion's own request limits sit far above any Cadence doc and return a loud `validation_error` rather than truncating.

**Invocation.** Two modes, one per write shape:
...
| Exit | Meaning |
|---|---|
| 2 | usage error: bad arguments, missing or empty content file, or `NOTION_TOKEN` unset (see [[#The authentication boundary]]) |
| 3 | pre-send guard: the body still carries an untranslated obsidian callout (`> [!` outside a code fence); translate per `skills/_shared/notion-translation.md` and re-invoke |
| 4 | API error: auth/permission/validation failure, rate-limit retries exhausted, or async task failure (message includes the failing status and a hint) |
| 5 | post-write verification failure: the read-back came up short; re-author from the source file and re-invoke |

The script sends `allow_async` and polls the task to completion (protection against Notion-side edge timeouts on large bodies), and retries rate limits itself.

**Interruption is recoverable.** Killing the script mid-run cannot poison later calls; there is no per-tool argument cache in play. The worst case is a half-written page, and the next `replace` overwrites it wholesale. After an interrupted `create`, check whether the sub-page already exists (via `resolve`/`read_artifact`) before re-creating, to avoid a duplicate.

**Small writes take the same path.** Uniformity is the point: no size threshold to judge, no second write path to drift. `notion-create-pages` body content and `notion-update-page` `insert_content` are not used for doc bodies at all; `notion-update-page` remains in use only for the small-argument operations bound above (`resolve_links` search-and-replace, `tick`, property edits), which sit far below the size band where the accumulator bug bites.
...
Two credentials exist on the notion branch, and Cadence stores neither.

**The MCP's credential** is entirely the Notion MCP's concern: the MCP holds and refreshes whatever token or integration credential it uses, and Cadence stores and passes no Notion secret. The one user-side requirement is sharing `root_page` with the Notion MCP's integration, so the integration can read the page and create databases and sub-pages beneath it; Cadence cannot grant this access on the user's behalf.

**The content write path's credential** is `NOTION_TOKEN`: the token of a Notion internal integration the user creates (https://www.notion.so/profile/integrations) and, like the MCP's integration, shares with `root_page` (sharing cascades to descendants). A Notion personal access token works too, since the REST markdown endpoints accept every connection type: a PAT acts as the user who created it and inherits their permissions, so no sharing step is needed at all. Internal integrations are the recommended default (least privilege, team-owned, each teammate isn't holding a person-scoped credential); a PAT is the low-friction choice for a solo user. Either way, `scripts/notion-write.js` reads the token from the environment only. When `NOTION_TOKEN` is unset, the script falls back to `NOTION_TOKEN_CMD`: a command whose stdout is the token, so the secret can live in a keychain or password manager instead of any plaintext file (e.g. `security find-generic-password -s notion-pat -w` on macOS, or a `op read` 1Password lookup). `NOTION_TOKEN` wins when both are set. The token value never appears in `.cadence/config.yaml`, `.cadence/config.local.yaml`, frontmatter, chat output, or logs. Missing both variables is a hard stop with an actionable message (the script's exit 2), the same shape as the missing-MCP hard fail: never degrade to an MCP content write, and never fall back to the filesystem.

When provisioning or a later operation fails because an integration lacks access to `root_page`, the remedy is the same either way: share the root page (and its descendants) with that integration.

...
| `write_doc(type, slug, slot, content)` | type, slug, slot id, markdown body | nothing | write `<slot>.md` in the folder | content authored in Notion-flavored Markdown (translate any obsidian callouts that slipped in — pre-send guard), upsert the slot's sub-page in ~8k-character chunks, verify on read-back |
...
**`write_doc(type, slug, slot, content)`** replaces one slot's content wholesale. The filesystem writes `<slot>.md` in the artifact's folder. On Notion, callouts are authored as native `<callout>` blocks from the start (per the authoring rule in `skills/_shared/notion-translation.md`); before the MCP call, `write_doc` runs that doc's pre-send guard — scan the body for `> [!` and translate any surviving obsidian callout — then hands the result to the official MCP, which creates the Notion blocks. A body over the size cap is sent as a sequence of chunked calls and verified on read-back before the operation returns; see [[#Content write size cap]]. Wikilinks resolve to mentions in the `resolve_links` second pass, not here. Partial edits are the caller's responsibility to assemble before the call; there is no append or patch form of this operation for callers. Chunking is internal to the notion backend and invisible above this layer.
...
### Content write size cap

**No single MCP content write carries more than ~8,000 characters of body content.** This is a hard rule on the notion branch, binding on every content-bearing call the layer makes: `notion-create-pages`, and `notion-update-page` in both its `insert_content` and `update_content` forms. It is a floor-level constraint, not a target: a write that would exceed it is split, never sent and hoped for.

**Why the cap exists.** The constraint is client-side, not Notion's. Notion's own per-request limits sit far above it (1000 block elements, 500KB payload, 2000 characters per rich-text element), and exceeding any of them returns a `validation_error` rather than a truncation. The binding constraint is the streaming tool-argument accumulator in Claude Code ([anthropics/claude-code#67765](https://github.com/anthropics/claude-code/issues/67765), open and unfixed): when the argument buffer ends mid-string, the in-flight string token is dropped and a bracket-closer stage repairs the JSON into something syntactically valid, so a short or mangled `content` field reaches the MCP and the call *succeeds*. A separate top-level backslash handler advances two characters instead of one, eating whatever follows a `\`. The result is silent tail corruption: literal `\n` or a bare `n` inside code fences, `*` characters swallowed, spurious `****`, and to-do items landing as escaped `- \[ \]` instead of real blocks. The failure is probabilistic and size-correlated (risk begins above roughly 4KB, and it is a gradient rather than a cliff), and 8KB is the largest size with direct positive evidence in the wild. Treat this cap as permanent rather than a temporary workaround.

**Chunked create-then-append.** A doc whose body exceeds the cap is written as a sequence:

1. **Create small.** Create the sub-page with `notion-create-pages` carrying only the head of the doc (title, frontmatter, and the first section), kept under the cap.
2. **Append the rest.** Extend the page with successive `notion-update-page` `insert_content` calls of ~8k characters each, in document order, each anchored after the content the previous call added.
3. **Split on block boundaries.** Cut between top-level blocks, never inside one. A chunk boundary must not land inside a fenced code block, a `<callout>`, a `<table>`, or a single list item; move the cut earlier to the nearest block boundary instead. A single block that alone exceeds the cap is a doc-authoring problem, not a chunking problem: surface it rather than splitting the block.
4. **Verify.** Run the post-write verification in `skills/_shared/notion-translation.md` before the operation returns.

This is the same two-pass shape as the existing `resolve_links` flow, and it stays entirely inside the notion backend: callers still see one `write_doc` call with one whole body.

**`allow_async: true` on large writes.** Set it on any write past the cap that Notion accepts it for; it returns a task handle polled via `notion-get-async-task`. It protects against Notion-side edge timeouts only. It does **not** address the client-side shear above and is never a reason to skip chunking or verification.

**Never interrupt a content write.** Pressing Escape mid-write is a deterministic trigger for the shear, and corrupted arguments are cached per-tool for the remainder of the session, so one interrupted write can poison every later `notion-create-pages` call. If a content write is interrupted for any reason, treat every subsequent write in that session as suspect: restart the session before continuing, and verify (read back) anything written after the interruption.

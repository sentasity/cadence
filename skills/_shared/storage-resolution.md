...
| `write_doc(type, slug, slot, content)` | type, slug, slot id, markdown body | nothing | write `<slot>.md` in the folder | content authored in Notion-flavored Markdown (translate any obsidian callouts that slipped in — pre-send guard), upsert the slot's sub-page |
